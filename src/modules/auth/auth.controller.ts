import { Controller, Get, Post, Body, UseFilters, UseGuards, Req, } from '@nestjs/common'; // Decoradores base de NestJS para 
// definir controladores, rutas y middlewares
import { AuthService } from './auth.service'; // Servicio de autenticación que contiene la lógica de negocio
import { AllExceptionsFilter } from 'src/common/filters/http-exception.filter'; // Filtro global de errores para capturar y 
// formatear excepciones HTTP
...
import type { RequestWithUser } from 'src/common/types/req-user.interface'; // Tipo extendido de Request con el usuario adjunto por 
// Passport
import { toUserResponseDto, UserResponseDto } from '../users/dto/user-response.dto'; // Helper que transforma la entidad User en un 
// DTO sin exponer el password
import { ApiResponse, ApiMessageResponse } from 'src/common/types/api-response.types'; // Tipos de respuesta estandarizados para 
// toda la API
...
export class AuthController {
  ...
  ): Promise<ApiResponse<{ user: UserResponseDto; accessToken: string }>> { // Devuelve una respuesta tipada ApiResponse con datos 
  // dinámicos (el usuario y el accessToken)
    const result = await this.authService.register(createUserDto); // Llama al servicio AuthService para crear el usuario y 
    // generar token
    return {
